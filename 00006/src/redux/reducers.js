...
const checkStreamTextKeyReducer = (state = false, action) => {
  if (action.type == 'FLIP_CHECK_STREAMTEXT_KEY')
    return !state
  else return state
}
...
const currentAPIReducer = (state = 0, action) => {
  if (action.type == "API_WEBSPEECH") {
    state = 0;
    return state
  } else if (action.type == "API_AZURE") {
    state = 1;
    return state
  } else if (action.type == "API_STREAMTEXT"){
    state = 2;
    return state
  } else {
    return state
...
const allReducers = combineReducers({
  ...
  checkStreamTextKey: checkStreamTextKeyReducer,
