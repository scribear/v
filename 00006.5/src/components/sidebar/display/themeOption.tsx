...
export default function Theme(props) {
    ...
    return (
        <div className="circle-picker " style={{display: 'flex', flexWrap: 'wrap', marginBottom: '-2vw', marginLeft: '.8vw' }}>
                <table>
                {Array.from(Array(4)).map((_, xIndex) => (
                    <tr>
                        {colors[xIndex].map((colors: color, yIndex) =>
                            <th>
                                <div style={{ width: '2.2vw', height: '2.2vw', marginRight: '.8vw', marginBottom: '.8vw', transform: 'scale(1)', transition: 'transform 100ms ease 0s' }}>
                                    <span>
                                        <button title="#000000" tabIndex={0} value = {JSON.stringify(colors)} onClick={changeTheme} style={{
